...
def transform():
    ...
    df['order_purchase_dayofweek']  = df['order_delivered_customer_date'].apply(lambda x: x.dayofweek)
    df['order_purchase_dayofweek_name'] = df['order_delivered_customer_date'].apply(lambda x: x.strftime('%a'))
